...
public class CarouselTemplate extends MessageLayoutTemplate {
    ...
    FrameLayout populateRichMessageContainer() {
        ...
        for (com.google.cloud.dialogflow.v2beta1.Context context : contextList) {
            contextParam = response.getQueryResult().getOutputContextsList().get(0).getParameters().getFieldsMap();
            cardList = (contextParam.get("carouselItems").getListValue() != null) ? contextParam.get("carouselItems").getListValue().getValuesList() : null;
//            cardList = [{"id":"1","imgUrl":"https://loremflickr.com/900/500/dog", "title":"<b>Image Title</b><i>(optional)</i>",
...
//            {"id":"3","imgUrl":"https://loremflickr.com/900/500/owl", "title":"<b>Image Title</b><i>(optional)</i>", "description":"<i>Image description. (optional)</i>","toast":"selected Owl"}];
            ...
            String align = response.getQueryResult().getOutputContextsList().get(0).getParameters().getFieldsMap().get("align").getStringValue();
            Log.d("Nazar", "align is : " + align);
            String sizeValue = response.getQueryResult().getOutputContextsList().get(0).getParameters().getFieldsMap().get("size").getStringValue();
                ...
                Runnable downloadImage = new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            bmp = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.introduction);
                        } catch (Exception e) {
